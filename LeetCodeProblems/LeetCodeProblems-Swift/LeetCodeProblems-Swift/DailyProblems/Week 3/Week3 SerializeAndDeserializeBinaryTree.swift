...
final class Week3SerializeAndDeserializeBinaryTree {
        ...
        func deserialize(_ data: String) -> TreeNode? {
            ...
            func parseInt() -> Int {
                ...
                while s[index].isNumber {
                    v = v * 10 + Int(s[index].asciiValue!) - 48
                    index += 1
                }
                return v * sign
            }
    ...
    class LevelOrderCodec2 {
        ...
        func deserialize(_ data: String) -> TreeNode? {
            ...
            guard let num = nums.first, let rootValue = num else { return nil }
    ...
    class PostorderCodec2 {
        func serialize(_ root: TreeNode?) -> String {
            func f(_ root: TreeNode?) -> String {
                root == nil ? "_," : f(root!.left) + f(root!.right) + "\(root!.val),"
            }
            return f(root)
        }
        ...
        func deserialize(_ data: String) -> TreeNode? {
            ...
            func build() -> TreeNode? {
                guard let num = nums.popLast(), let v = num else { return nil }
                let node = TreeNode(v)
                node.right = build()
                node.left = build()
                return node
    ...
    class PreorderCodec2 {
        func serialize(_ root: TreeNode?) -> String {
            func f(_ root: TreeNode?) -> String {
                root == nil ? "_," : "\(root!.val)," + f(root!.left) + f(root!.right)
            }
            return f(root)
        }
