    ...
    final class MaxProfit2 {
        static func run() {
            func judge(prices: [Int], expected: Int) {
                printAndAssert(result: greedy1(prices), expected: expected)
            }
        ...
        static func greedy1(_ prices: [Int]) -> Int {
            (1..<prices.count).reduce(into: 0) { sum, i in
                sum += max(prices[i] - prices[i - 1], 0)
            }
        }

        static func compressedDP1(_ prices: [Int]) -> Int {
        ...
        static func rawDP1(_ prices: [Int]) -> Int {
